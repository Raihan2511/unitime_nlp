...
class UniTimeNLPEvaluator:
    def __init__(self, model_dir="output/final_model"):
    ...
    def predict(self, input_text, max_length=512):
        ...
        inputs = self.tokenizer([input_text], return_tensors="pt", truncation=True, padding=True, max_length=max_length)
        with torch.no_grad():
    ...
    def evaluate_on_test_set(self, test_path="data/processed/test_dataset.json", output_path="output/evaluation_results.json"):
        ...
        for i, example in enumerate(test_data):
            print(f"\nEvaluating example {i+1}/{len(test_data)}")
            pred = self.predict(example["input"])
            predictions.append({
                "input": example["input"],
